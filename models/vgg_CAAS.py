...
class DRS(nn.Module):
    ...
    """
    def __init__(self, delta, channel, num_classes=20):
    ...
    def forward(self, xr):
        ...
        """ 1: max extractor """
        x_max = self.global_max_pool(x).view(b,c)

        route_outs = torch.cat((route_outs, x_max), 1)

        # print('x_max.shape: ', x_max.shape)
        # print('classifier_route: ', self.classifier_route)
        route_classifier_out = self.classifier_route(x_max)
...
class VGG(nn.Module):
    def __init__(self, features, delta=0, num_classes=20, init_weights=True):
        ...
        self.layer1_relu1 = DRS_learnable(64) if delta == 0 else DRS(delta, 64)
        self.layer1_conv2 = features[2]
        self.layer1_relu2 = DRS_learnable(64) if delta == 0 else DRS(delta, 64)
        self.layer1_maxpool = features[4]
        
        self.layer2_conv1 = features[5]
        self.layer2_relu1 = DRS_learnable(128) if delta == 0 else DRS(delta, 128)
        self.layer2_conv2 = features[7]
        self.layer2_relu2 = DRS_learnable(128) if delta == 0 else DRS(delta, 128)
        self.layer2_maxpool = features[9]
        
        self.layer3_conv1 = features[10]
        self.layer3_relu1 = DRS_learnable(256) if delta == 0 else DRS(delta, 256)
        self.layer3_conv2 = features[12]
        self.layer3_relu2 = DRS_learnable(256) if delta == 0 else DRS(delta, 256)
        self.layer3_conv3 = features[14]
        self.layer3_relu3 = DRS_learnable(256) if delta == 0 else DRS(delta, 256)
        self.layer3_maxpool = features[16]
        
        self.layer4_conv1 = features[17]
        self.layer4_relu1 = DRS_learnable(512) if delta == 0 else DRS(delta, 512)
        self.layer4_conv2 = features[19]
        self.layer4_relu2 = DRS_learnable(512) if delta == 0 else DRS(delta, 512)
        self.layer4_conv3 = features[21]
        self.layer4_relu3 = DRS_learnable(512) if delta == 0 else DRS(delta, 512)
        self.layer4_maxpool = features[23]
        
        self.layer5_conv1 = features[24]
        self.layer5_relu1 = DRS_learnable(512) if delta == 0 else DRS(delta, 512)
        self.layer5_conv2 = features[26]
        self.layer5_relu2 = DRS_learnable(512) if delta == 0 else DRS(delta, 512)
        self.layer5_conv3 = features[28]
        self.layer5_relu3 = DRS_learnable(512) if delta == 0 else DRS(delta, 512)
        
        self.extra_conv1 = nn.Conv2d(512, 512, kernel_size=3, padding=1)
        self.extra_relu1 = DRS_learnable(512) if delta == 0 else DRS(delta, 512)
        self.extra_conv2 = nn.Conv2d(512, 512, kernel_size=3, padding=1)
        self.extra_relu2 = DRS_learnable(512) if delta == 0 else DRS(delta, 512)
        self.extra_conv3 = nn.Conv2d(512, 512, kernel_size=3, padding=1)
        self.extra_relu3 = DRS_learnable(512) if delta == 0 else DRS(delta, 512)
    ...
    def forward(self, x, label=None, size=None):
        ...
        xr = [x, route_outs, route_classifier_outs]
        xr = self.layer2_relu1(xr)
        x, route_outs, route_classifier_outs = xr[0], xr[1], xr[2]

        x = self.layer2_conv2(x)

        xr = [x, route_outs, route_classifier_outs]
        xr = self.layer2_relu2(xr)
        x, route_outs, route_classifier_outs = xr[0], xr[1], xr[2]
        ...
        xr = [x, route_outs, route_classifier_outs]
        xr = self.layer3_relu1(xr)
        x, route_outs, route_classifier_outs = xr[0], xr[1], xr[2]

        x = self.layer3_conv2(x)

        xr = [x, route_outs, route_classifier_outs]
        xr = self.layer3_relu2(xr)
        x, route_outs, route_classifier_outs = xr[0], xr[1], xr[2]

        x = self.layer3_conv3(x)

        xr = [x, route_outs, route_classifier_outs]
        xr = self.layer3_relu3(xr)
        x, route_outs, route_classifier_outs = xr[0], xr[1], xr[2]
        ...
        xr = [x, route_outs, route_classifier_outs]
        xr = self.layer4_relu1(xr)
        x, route_outs, route_classifier_outs = xr[0], xr[1], xr[2]

        x = self.layer4_conv2(x)

        xr = [x, route_outs, route_classifier_outs]
        xr = self.layer4_relu2(xr)
        x, route_outs, route_classifier_outs = xr[0], xr[1], xr[2]

        x = self.layer4_conv3(x)

        xr = [x, route_outs, route_classifier_outs]
        xr = self.layer4_relu3(xr)
        x, route_outs, route_classifier_outs = xr[0], xr[1], xr[2]
        ...
        xr = [x, route_outs, route_classifier_outs]
        xr = self.layer5_relu1(xr)
        x, route_outs, route_classifier_outs = xr[0], xr[1], xr[2]

        x = self.layer5_conv2(x)

        xr = [x, route_outs, route_classifier_outs]
        xr = self.layer5_relu2(xr)
        x, route_outs, route_classifier_outs = xr[0], xr[1], xr[2]

        x = self.layer5_conv3(x)

        xr = [x, route_outs, route_classifier_outs]
        xr = self.layer5_relu3(xr)
        x, route_outs, route_classifier_outs = xr[0], xr[1], xr[2]
        ...
        xr = [x, route_outs, route_classifier_outs]
        xr = self.extra_relu1(xr)
        x, route_outs, route_classifier_outs = xr[0], xr[1], xr[2]

        x = self.extra_conv2(x)

        xr = [x, route_outs, route_classifier_outs]
        xr = self.extra_relu2(xr)
        x, route_outs, route_classifier_outs = xr[0], xr[1], xr[2]

        x = self.extra_conv3(x)

        xr = [x, route_outs, route_classifier_outs]
        xr = self.extra_relu3(xr)
        x, route_outs, route_classifier_outs = xr[0], xr[1], xr[2]
